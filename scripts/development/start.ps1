...
function Wait-ServicesReady {
  ...
  # 从环境变量读取端口号
  $managementServerPort = [System.Environment]::GetEnvironmentVariable("MANAGEMENT_SERVER_PORT") ?? "8001"
  $vueFrontendPort = [System.Environment]::GetEnvironmentVariable("WEB_UI_PORT") ?? "5174"
  ...
  while (-not $fastApiReady -and $retryCount -lt $maxRetries) {
    try {
      # 先检查端口是否开放
      $tcpClient = New-Object System.Net.Sockets.TcpClient
      ...
      # 尝试调用健康检查端点
      try {
        $response = Invoke-RestMethod -Uri "http://localhost:8000/health" -Method Get -ErrorAction Stop -TimeoutSec 5
        ...
        Write-ColorOutput "端口开放但无法获取健康状态，$retryCount/$maxRetries 次重试..." "Warning"
        $retryCount++
        Start-Sleep 3
      }
    }
  ...
  if (-not $fastApiReady) {
    Write-ColorOutput "⚠ FastAPI 服务器启动超时" "Error"
...
function Show-Status {
  ...
  Write-ColorOutput "• FastAPI 管理服务器: http://localhost:8000" "Info"
